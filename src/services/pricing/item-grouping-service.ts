...
export default class ItemGroupingService {
  ...
  public async startWriteJob() {
    for (;;) {
      const itemGroups: ItemGroupInfo[] = Object.values(this.itemGroupsToWrite);
      this.itemGroupsToWrite = {};
      if (itemGroups.length > 0) {
        for (const itemGroup of itemGroups) {
          try {
            await this.postgresService.prisma.itemGroupInfo.upsert({
              where: { hashString: itemGroup.hashString },
              create: itemGroup,
              update: { ...itemGroup, createdAtTimestamp: undefined },
            });
          } catch (error) {
            Logger.error("error in item group write job", error);
          }
        }
      }
...
export class HeistBlueprintsGroupIdentifier implements ItemGroupIdentifier {
  group(item: PoeApiItem): InternalGroup {
    if (item.frameType === 3 || item.frameType === 10) return;
...
export class InucbatorGroupIdentifier implements ItemGroupIdentifier {
  ...
  group(item: PoeApiItem): InternalGroup {
    const baseType = item.baseType?.toLowerCase();
    if (this.incubatorBaseTypes.has(baseType)) {
      const ilvl = item["ilvl"] ?? item.itemLevel;
      const group: InternalGroup = {
        key: baseType,
        tag: "incubator",
...
export class CurrencyGroupIdentifier implements ItemGroupIdentifier {
  private readonly currenyGropIds = {
    currency: {
      items: new Set([
        "gold",
        "chaos orb",
  ...
  group(item: PoeApiItem): InternalGroup {
    ...
    for (const [key, value] of Object.entries(this.currenyGropIds)) {
